...
def get_results_by_date(screener_name, run_date):
    """Get results by screener and date - supports both DB and file-based results"""
    # Check if it's a realtime result (file-based)
    if run_date.startswith('realtime-'):
        return get_realtime_results_from_file(screener_name, run_date)
    
    # Regular DB-based result
    run = get_run(screener_name, run_date)
    if not run:
        return None
    return get_results(run['id'])

def get_realtime_results_from_file(screener_name, run_date):
    """从文件读取 realtime 结果"""
    from pathlib import Path
    import pandas as pd
    
    workspace_root = Path('/Users/mac/.openclaw/workspace-neo')
    dir_name = screener_name.replace('_screener', '')
    
    # 尝试读取 CSV 文件
    csv_path = workspace_root / 'data' / 'screeners' / dir_name / f'{run_date}.csv'
    ...
    try:
        ...
        for _, row in df.iterrows():
            result = {
                ...
                'pct_change': float(row.get('pct_change', 0) or row.get('change_ratio', 0) or 0),
        ...
        print(f"Error reading realtime file: {e}")
        return None
