...
def is_valid_screener_file(filepath):
    ...
    try:
        ...
        # Look for class definition ending with 'Screener' (with or without parenthesis)
        if not re.search(r'class\s+\w+Screener\s*[\(:]', content):
            return False
        return True
    except Exception:
        return False
...
def extract_screener_info(filepath):
    ...
    try:
        ...
        # Extract class name
        class_match = re.search(r'class\s+(\w+Screener)\s*[\(:]', content)
...
def discover_screeners():
    ...
    search_paths = [WORKSPACE_ROOT, WORKSPACE_ROOT / "scripts"]
...
def get_screener_class_name(screener_name):
    ...
    # 先在根目录查找，再在 scripts 目录查找
    filepath = WORKSPACE_ROOT / f"{screener_name}.py"
    if not filepath.exists():
        filepath = WORKSPACE_ROOT / "scripts" / f"{screener_name}.py"
    
    if not filepath.exists():
        raise FileNotFoundError(f"Screener file not found: {screener_name}.py")
    
    try:
        content = filepath.read_text(encoding='utf-8')
        # Look for class definition ending with 'Screener' (with or without parenthesis)
        match = re.search(r'class\s+(\w+Screener)\s*[\(:]', content)
        if match:
...
def run_screener_subprocess(screener_name, run_date):
    ...
    try:
        ...
        # 先在根目录查找，再在 scripts 目录查找
        screener_file = WORKSPACE_ROOT / f"{screener_name}.py"
        if not screener_file.exists():
            screener_file = WORKSPACE_ROOT / "scripts" / f"{screener_name}.py"
        
        if not screener_file.exists():
            raise FileNotFoundError(f"Screener file not found: {screener_file}")
        ...
        spec = importlib.util.spec_from_file_location(screener_name, screener_file)
        ...
        # Some screeners return {'hot': [...], 'cold': [...]}
        # Others return [...] or (results, analysis_data) tuple
        ...
        for r in results:
            # 支持多种价格字段名
            close_price = r.get('close', r.get('current_price', r.get('price', 0)))
            extra_data = {k: v for k, v in r.items() 
                         if k not in ['code', 'name', 'close', 'current_price', 'price', 'turnover', 'pct_change']}
        ...
        # Mark as completed
        complete_run(run_id, stocks_found=len(results))
...
def get_stock_data_for_chart(stock_code, days=60):
    ...
    try:
        ...
        for row in reversed(rows):  # Reverse to get ascending order
            records.append({
                'date': row[0],
                'open': round(row[1], 2) if row[1] else 0,
                'high': round(row[2], 2) if row[2] else 0,
                'low': round(row[3], 2) if row[3] else 0,
                'close': round(row[4], 2) if row[4] else 0,
                'volume': int(row[5]) if row[5] else 0,
                'amount': round(row[6], 2) if row[6] else 0
            })
