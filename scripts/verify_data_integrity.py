...
class DataIntegrityChecker:
    ...
    def _load_expected_trading_days(self):
        ...
        self.expected_trading_days = trading_days
        logger.info(f"📅 预期交易日: {len(trading_days)} 天 ({self.start_date} 至 {self.end_date})")
    
    def get_all_stocks(self) -> List[Tuple[str, str, int]]:
        """获取所有股票代码、名称和退市状态"""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.execute(
            "SELECT code, name, is_delisted FROM stocks ORDER BY code"
        )
